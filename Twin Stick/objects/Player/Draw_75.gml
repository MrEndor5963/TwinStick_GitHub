...
glitch_sprite(portrait_sprite,(draw_gui_origin_x+(48*pon_x)),(draw_gui_origin_y+(48*pon_y)))

bktglitch_set_intensity(0.04 + (0));
glitch_sprite(s_HPBarOutline,draw_gui_origin_x+(48*pon_x),draw_gui_origin_y+(pon_y*192))

bktglitch_set_intensity(0.04 + (glitch_int_gun_name));
glitch_text(draw_gui_origin_x+(96*pon_x),draw_gui_origin_y+(24*pon_y),string(weapon_name))
...
glitch_text(draw_gui_origin_x+(96*pon_x)-xoff,draw_gui_origin_y+(72*pon_y),string(ammo_inmag))

if pon_x = -1{xoff = string_width(string(ammo_inmag))}
bktglitch_set_intensity(0.04 + (glitch_int_reserve));
glitch_text(draw_gui_origin_x+((96+string_width(ammo_inmag))*pon_x)+xoff,draw_gui_origin_y+(72*pon_y),"/"+string(ammo_reserve))

if pon_x = -1{xoff = string_width("$"+string(money))}
bktglitch_set_intensity(0.04 + (glitch_int_points));
glitch_text(draw_gui_origin_x+(0*pon_x),draw_gui_origin_y+(122*pon_y),"$"+string(money))
